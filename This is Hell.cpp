...
bool fever = false;
//const double scoreBoardWidth = 60;
SDL_Rect scoreBoard = {viewPortWidth-scoreBoardWidth, windowHeight -50, scoreBoardWidth, 50};
SDL_Rect comboNotif = {viewPortWidth/2-50, windowHeight/2, 100, 40};
SDL_Rect realWorldBackground = {0,0,viewPortWidth, windowHeight};
...
string savedHighScore[35];
int highestScores[3];
string bestPlayers[3];
int longestCombos[3];
string bestComboPlayers[3];
...
SDL_Texture *feverBarStaticTex = NULL;
SDL_Texture *realWorldBackgroundTex = NULL;
SDL_Texture *songSelectOneTex = NULL;
SDL_Texture *songSelectTwoTex = NULL;
SDL_Texture *songSelectThreeTex = NULL;
SDL_Texture *songSelectFourTex = NULL;
...
SDL_Rect wrongPress = {0, 160, 200, 80};
...
bool loadMedia()
{
    ...
    bool success = true;
 	...
 	soulTex = loadTexture("soul.png"); //use airshipBounded for visible boundaries
    if( soulTex == NULL )
    {
	...
	dyingBodyTex = loadTexture("dyingbody.png");
	...
	soulThresholdTex = loadTexture("strumbar.png");
	...
	onBeatTex = loadTexture("onbeat.png");
	if( onBeatTex == NULL )
	{
		printf( "Failed to load texture image!\n" );
        success = false;
	...
	feverBarTex = loadTexture("feverbarfill.png");
	if( feverBarTex == NULL )
	{
		printf( "Failed to load texture image!\n" );
        success = false;
	...
	currentLaneTex = loadTexture("lanehighlight.png");
	...
	crosshairTex = loadTexture("cursor.png"); //use airshipBounded for visible boundaries
	...
	laserActiveTex = loadTexture("laseractive.png");
	if( laserActiveTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
    }

	laserStockReadyTex = loadTexture("laserstockready.png");
	if( laserStockReadyTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
    }

	laserStockNotReadyTex = loadTexture("laserstocknotready.png");
	if( laserStockNotReadyTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
    }

	groundTex = loadTexture("obstacle.png"); //floor.png
	if( groundTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
	...
	gunLockTex = loadTexture("cursoroverheat.png");
	if( gunLockTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
	...
	gunHeatTex = loadTexture("gunheat.png");
	if( gunLockTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
	...
	gunOverheatTex = loadTexture("gunoverheat.png");
	if( gunLockTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
	...
	gunHeatFrameTex = loadTexture("gunheatframe.png");
	if( gunLockTex == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
	...
	buttonPressTex = loadTexture("whack a soul.png");
	...
	feverBarStaticTex = loadTexture("gunheatframe.png");
	if (feverBarStaticTex == NULL)
	{
		printf("Failed to load feverBarStaticTex!\n");
		success = false;
	}

	realWorldBackgroundTex = loadTexture("hospital background.png");
	if (realWorldBackgroundTex == NULL)
	{
		printf("failed to load realWorldBackgroundTex!\n");
		success=false;
	}

	songSelectOneTex = loadTexture("songOne.png");
	if (songSelectOneTex == NULL)
	{
		printf("failed to load song select!\n");
		success=false;
	}
	songSelectTwoTex = loadTexture("songTwo.png");
	if (songSelectTwoTex == NULL)
	{
		printf("failed to load song select!\n");
		success=false;
	}
	songSelectThreeTex = loadTexture("songThree.png");
	if (songSelectThreeTex == NULL)
	{
		printf("failed to load song select!\n");
		success=false;
	}
	songSelectFourTex = loadTexture("songFour.png");
	if (songSelectFourTex == NULL)
	{
		printf("failed to load song select!\n");
		success=false;
	}
...
void playGame()
{
	...
	while (running)	
	{
		...
		SDL_RenderCopy(ren, realWorldBackgroundTex, NULL, &realWorldBackground);
		for (int i = 0; i <numberOfSouls; i++)
		{
		...
		SDL_RenderCopy(ren, feverBarStaticTex, NULL, &feverBarStatic);
		SDL_RenderCopy(ren, soulThresholdTex, NULL, &hitZone);
		if (!playerStrum1)
		{
			SDL_RenderCopy(ren, buttonPressTex, &noPress, &firstWhacker);
		}
		if (!playerStrum2)
		{
			SDL_RenderCopy(ren, buttonPressTex, &noPress, &secondWhacker);
		}
		if (!playerStrum3)
		{
			SDL_RenderCopy(ren, buttonPressTex, &noPress, &thirdWhacker);
		}
		//checkAndRenderButtonPress();
		...
		if(currentSound->isFinished() == false)
		{
			...
			if(soulNotes[soulSequence[sequenceID]].y == dyingBody && sequenceID < sequenceCount)
			{
				...
				else if(sequenceID != 0)
				{
					...
					//SDL_RenderCopy(ren, buttonPressTex, &wrongPress, &buttonPress[soulSequence[sequenceID]]);
				}
				sequenceID++;
			}
		}
		else 
		{
			if(scoreRecorded == false)// && soulNotes[soulSequence[sequenceID]].y == dyingBody)
			{
				...
				myfile << " " << endl;
				myfile << "Score 1:" <<endl;
				myfile << highestScores[0] << endl;
				myfile << "By: " << endl;
				myfile << bestPlayers[0] << endl;
				myfile << " " << endl;
				myfile << "Score 2:" <<endl;
				myfile << highestScores[1] << endl;
				myfile << "By: " << endl;
				myfile << bestPlayers[1] << endl;
				myfile << " " << endl;
				myfile << "Score 3:" <<endl;
				myfile << highestScores[2] << endl;
				myfile << "By: " << endl;
				myfile << bestPlayers[2] << endl;
				myfile << " " << endl;
				myfile << "-----------------------LONGEST COMBOS---------------------" << endl;
				myfile << " " << endl;
				myfile << "Longest Combo 1: " << endl;
				myfile << longestCombos[0] << endl;
				myfile << "By: " << endl;
				myfile << bestComboPlayers[0] << endl;
				myfile << " " << endl;
				myfile << "Longest Combo 2: " << endl;
				myfile << longestCombos[1] << endl;
				myfile << "By: " << endl;
				myfile << bestComboPlayers[1] << endl;
				myfile << " " << endl;
				myfile << "Longest Combo 3: " << endl;
				myfile << longestCombos[2] << endl;
				myfile << "By: " << endl;
				myfile << bestComboPlayers[2] << endl;
				myfile << " " << endl;
				myfile.close();
				scoreRecorded = true;
		...
		if (laserOn)
		{
			SDL_RenderCopyEx(ren, laserActiveTex, NULL, &laserPos, -gameFrame*3, NULL, SDL_FLIP_NONE);
		}
		if (!gunLock)
		{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				SDL_RenderCopy(ren, laserActiveTex, NULL, &laserCharges[i]);
			}
			else if (laserStock[i] && !laserOn)
			{
				SDL_RenderCopy(ren, laserStockReadyTex, NULL, &laserCharges[i]);
			}
			else
			{
				SDL_RenderCopy(ren, laserStockNotReadyTex, NULL, &laserCharges[i]);
			}
		}
...
int main(int argc, char* argv[]) 
{
	...
	SDL_Rect firstOption = {windowWidth/3, windowHeight/2+50, windowWidth/3, 100};
	...
	{	
		...
		SDL_RenderCopy(ren, songSelectThreeTex, NULL, &thirdOption);
		SDL_RenderCopy(ren, songSelectFourTex, NULL, &fourthOption);
